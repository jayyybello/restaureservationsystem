...
// Check if user is trying to log out (must be before output)mysqli_close
 ...
 // --- Handle User Actions (delete_user, add_user, edit_user) ---
    ...
    elseif ($action === 'delete_user' && isset($_POST['user_id'])) { // --- ADDED THIS BLOCK ---
        ...
        if (!check_permission('admin')) {
            $_SESSION['error'] = "You don't have permission to edit tables.";
            redirect_with_message('tables', 'error', 'You don\'t have permission to edit tables.');
        }

        $table_id = filter_input(INPUT_POST, 'table_id', FILTER_VALIDATE_INT);
        $physical_table_id = filter_input(INPUT_POST, 'physical_table_id', FILTER_VALIDATE_INT);
        $capacity = filter_input(INPUT_POST, 'capacity', FILTER_VALIDATE_INT);
        $location = htmlspecialchars($_POST['location'], ENT_QUOTES, 'UTF-8');
        $availability = htmlspecialchars($_POST['availability'], ENT_QUOTES, 'UTF-8');

        if (!$table_id || !$physical_table_id || !$capacity || !$location || !$availability) {
            redirect_with_message('tables', 'error', 'All fields are required.');
        }

        // Validate capacity
        if ($capacity < 1) {
            redirect_with_message('tables', 'error', 'Capacity must be at least 1.');
        }

        // Validate availability
        $valid_availabilities = ['available', 'taken', 'unavailable'];
        if (!in_array($availability, $valid_availabilities)) {
            redirect_with_message('tables', 'error', 'Invalid availability status.');
        }

        // Check if physical table ID already exists for a different table
        $check_sql = "SELECT id FROM table_mapping WHERE physical_table_id = ? AND id != ?";
        $check_stmt = $conn->prepare($check_sql);
        $check_stmt->bind_param("ii", $physical_table_id, $table_id);
        $check_stmt->execute();
        $check_result = $check_stmt->get_result();
        
        if ($check_result->num_rows > 0) {
            redirect_with_message('tables', 'error', 'A table with this Physical Table ID already exists.');
        }

        // Update the table
        $sql = "UPDATE table_mapping SET 
                physical_table_id = ?, 
                capacity = ?, 
                location = ?, 
                availability = ?
        ...
        if ($stmt->execute()) {
            ...
            redirect_with_message('tables', 'error', 'Error updating table: ' . $conn->error);
        }
     } elseif ($action === 'edit_reservation') {
        if (!check_permission('admin')) {
            redirect_with_message('reservations', 'error', 'You don\'t have permission to edit reservations.');
        }
        ...
        $people_count = filter_input(INPUT_POST, 'people_count', FILTER_VALIDATE_INT);
        $status = filter_input(INPUT_POST, 'status', FILTER_SANITIZE_SPECIAL_CHARS);
        $special_requests = htmlspecialchars($_POST['special_requests'] ?? '', ENT_QUOTES, 'UTF-8');

        if (!$reservation_id || !$name || !$email || !$phone || !$date || !$time || !$people_count || !$status) {
            redirect_with_message('reservations', 'error', 'All required fields must be filled out.');
        }

        // Validate people count
        if ($people_count < 1) {
            redirect_with_message('reservations', 'error', 'Party size must be at least 1.');
        }

        // Validate status
        $valid_statuses = ['pending', 'confirmed', 'cancelled', 'completed'];
        if (!in_array($status, $valid_statuses)) {
            redirect_with_message('reservations', 'error', 'Invalid status value.');
        }

        // Update the reservation
        $sql = "UPDATE table_booking SET 
                name = ?,
                email = ?,
                phone = ?,
                date = ?, 
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssi", $name, $email, $phone, $date, $time, $people_count, $status, $special_requests, $reservation_id);
        
        if ($stmt->execute()) {
            redirect_with_message('reservations', 'success', 'Reservation updated successfully!');
        } else {
            redirect_with_message('reservations', 'error', 'Error updating reservation: ' . $conn->error);
        }
     }
